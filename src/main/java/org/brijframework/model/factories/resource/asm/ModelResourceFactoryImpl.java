...
public abstract class ModelResourceFactoryImpl<K, T extends ModelResource<?>> extends AbstractFactory<K, T> implements MetaFactory<K, T>{
	
	private ConcurrentHashMap<K, T> cache = new ConcurrentHashMap<>();
	
	private Container container;
	
	protected ModelResourceFactoryImpl() {
	}
	...
	@Override
	public ModelResourceFactoryImpl<K, T> clear() {
		if (getCache() != null) {
			getCache().clear();
		}
		return this;
	...
	@Override
	public ModelResourceFactoryImpl<K, T> loadFactory() {
		return this;
	}
